...
func load_from_file():
    ...
    if dict_from_file.has("audio_settings"):
        ...
        audio_settings = load("res://globals/settings/defaults/default_audio_settings.tres") as AudioSettings

    if dict_from_file.has("game_settings"):
        game_settings = GameSettings.from_dict(dict_from_file.get("game_settings"))
    else:
        game_settings = load("res://globals/settings/defaults/default_game_settings.tres") as GameSettings
