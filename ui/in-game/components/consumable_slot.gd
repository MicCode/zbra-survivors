...
func _ready() -> void:
    PlayerService.consumable_changed.connect(_update_slot)
    PlayerService.consumable_use_changed.connect(func(time_used: int):
        %UseLabel.text = str("%d" % (max_uses - time_used))
    )
    _update_slot(PlayerService.consumable)
