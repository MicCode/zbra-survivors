...
static func apply_modifiers(base_stats: WeaponStats, modifiers: Array[StatModifier]) -> WeaponStats:
    var new_stats: WeaponStats = base_stats.duplicate(true)
    for mod in modifiers.filter(func(m: StatModifier): return m.is_type(E.ModType.WEAPON)):
        var stat_name = mod.get_stat_name()
        if new_stats.get(stat_name) == null:
            push_error("Unable to apply weapon stat modifier, stat name [%s] not found in class WeaponStats" % stat_name)
        else:
            var base_value = base_stats.get(stat_name)
            if typeof(base_value) != typeof(mod.value):
                push_error("WeaponStats property [%s] is expecting type [%d] but provided modifier is of type [%d]" % [stat_name, typeof(base_value), typeof(mod.modifier_value)])
            else:
                if mod.is_absolute:
